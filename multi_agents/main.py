...
async def run_research_task(query, websocket=None, stream_output=None, tone=Tone.Objective, headers=None):
    ...
    task["query"] = query
    #task["source"] = source
    task["report_style"] = "detailed report"
    #task["source_urls"] = source_urls
    include_domains, system_instructions, search_query_instructions = await get_secialisation_config("investment")
...
async def get_secialisation_config(agent_specialization):
    ...
    return include_domains, system_instructions, search_query_instructions
