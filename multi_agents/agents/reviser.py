...
You MUST only consider the data provided in the Draft and review notes. Do not use external sources to review the draft.
"""
        }]
    ...
    async def run(self, draft_state: dict):
        ...
        if draft_state.get("task").get("verbose"):
            if self.websocket and self.stream_output:
                await self.stream_output("logs", "revision_notes", f"Revision notes for Query:\n'{query}' and sub-topic:\n'{topic}' is:\n{revision.get('revision_notes')}", self.websocket)
            else:
                print_agent_output(f"Revision notes for Query: '{query}' and sub-topic: '{topic}':\n{revision.get('revision_notes')}", agent="REVISOR")
